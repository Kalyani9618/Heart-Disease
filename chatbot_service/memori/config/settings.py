...
class DatabaseSettings(BaseModel):
    ...
    connection_string: str = Field(
        default="postgresql://postgres:postgres@localhost:5432/memori",
...
class AgentSettings(BaseModel):
    ...
    # API Key - supports OpenAI, OpenRouter, or custom providers
    openai_api_key: str | None = Field(
        default=None, description="API key for LLM provider (OpenAI, OpenRouter, etc.)"
    )
    
    # OpenRouter/Custom Provider Configuration
    llm_provider: str = Field(
        default="openrouter", description="LLM provider: 'openai', 'openrouter', 'llama-local', 'custom'"
    )
    llm_base_url: str | None = Field(
        default="https://openrouter.ai/api/v1", description="Base URL for LLM API (OpenRouter, local, etc.)"
    )
    
    default_model: str = Field(
        default="openai/gpt-4o-mini", description="Default model for memory processing"
    )
    fallback_model: str = Field(
        default="openai/gpt-3.5-turbo", description="Fallback model if default fails"
    )
    ...
    def validate_api_key(cls, v):
        """Validate API key format - supports OpenAI (sk-) and OpenRouter (sk-or-) keys"""
        if v:
            # Accept OpenAI keys (sk-), OpenRouter keys (sk-or-), or any non-empty key for custom providers
            if not (v.startswith("sk-") or len(v) > 10):
                raise ValueError("API key appears invalid. Expected OpenAI (sk-*), OpenRouter (sk-or-*), or valid custom key")
        return v
